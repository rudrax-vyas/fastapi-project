...
class Child(Base):
    ...
    parent: Mapped['Parent'] = relationship(back_populates='children', cascade='expunge')
    
    
    def __repr__(self):
        return f'<Child - parent_id: {self.parent_id}>'
    
engine = create_engine('postgresql://admin:admin123@localhost:15432/mydb')
...
try:
    ...
    print('parent.children are not in session')
